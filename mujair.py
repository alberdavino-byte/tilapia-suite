        ...
        if not email:
            ...
            try:
                with st.spinner("Mengirim email..."):
                    ...
                    email,
                     {"redirect_to": app_url + "/auth/v1/callback"}
                    )
    ...
    if submitted:
        if not new_password or not confirm_password:
            ...
            if not is_valid:
               ...
               if st.button("Ubah Password", type="primary"):
                if new_password != confirm_password:
                 st.error("Password tidak sama")
    else:
        try:
            with st.spinner("Mengubah password..."):
                # wajib! set session dengan recovery_token
                ...
                # bersihkan token dan redirect ke halaman main (login)
