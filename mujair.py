...
def handle_verification():
    ...
    try:
        ...
        # 🔹 CASE 2: Reset Password (type=recovery)
            ...
            # simpan token dan arahkan ke halaman reset
            st.session_state.recovery_token = access_token
            st.session_state.page = "reset_password"

            # ⚠️ jangan hapus query param di sini — biarkan terbawa ke rerun
            st.experimental_rerun()

        # 🔹 CASE 3: Tipe tidak dikenal
        else:
...
def forgot_password_page(supabase: Client):
    ...
    st.write("Redirect URL:", f"{get_app_url()}/?type=recovery")
