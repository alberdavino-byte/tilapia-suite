...
def handle_verification():
    """Handle callback verifikasi email dan reset password dari Supabase"""
    params = dict(st.query_params)
    if not params or "access_token" not in params:
        return  # Tidak ada query param dari Supabase
    ...
    try:
        # 🔹 CASE 1: Verifikasi Email (signup)
        if type_ == "signup" or refresh_token:
            supabase.auth.set_session(access_token, refresh_token)
            user = supabase.auth.get_user()

            if user and user.user:
                supabase.table("users").update({"is_verified": True}).eq("id", user.user.id).execute()
                st.success("✅ Email berhasil diverifikasi! Silakan login kembali.")
        ...
        # 🔹 CASE 2: Reset Password (type=recovery)
        elif type_ == "recovery":
            # 🚨 Perbaikan penting: jangan hapus query param sebelum rerun
            st.session_state.recovery_token = access_token
            st.session_state.page = "reset_password"
            st.rerun()  # biarkan query param masih ada saat rerun

            # setelah rerun baru boleh dihapus (tidak akan dieksekusi di rerun pertama)
            st.query_params.clear()

        else:
            st.warning(f"⚠️ Jenis verifikasi tidak dikenali: {type_}")
            st.query_params.clear()

    except Exception as e:
        st.error(f"❌ Gagal memproses verifikasi/reset password: {e}")
        st.query_params.clear()
# Halaman Register
        ...
        try:
            with st.spinner("Memproses login..."):
                ...
                if user_data.data['role'] != role:
                    st.error(f"❌ Anda terdaftar sebagai **{user_data.data['role']}**, bukan **{role}**.")
                    supabase.auth.sign_out()
                    return
...
def reset_password_page(supabase: Client):
    ...
    if submitted:
        ...
        try:
            ...
            st.info("Silakan login kembali dengan password baru.")
            ...
            st.info("💡 Token mungkin sudah Expired. Kirim reset password lagi.")
...
def main():
    ...
    if st.session_state.page == 'forgot_password':  # ← DITAMBAHKAN
